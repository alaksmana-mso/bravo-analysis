...
**One thing survives — and as of 2026-09-11 it is Bravo-specific, not platform-neutral.** In both samples essentially every `Story`-type issue is assigned to a single account while sub-tasks distribute across the rest — 9 of 9 long-lived Stories in June, 9 of 11 in August. Whoever holds the shape of a Bravo change today is a **bus factor of one**, on a service carrying ~94% of origination. That is a live operational risk needing an owner **whatever is decided about new products**. LORA's Stories are spread across a dozen accounts in the same samples.

> **Answered 2026-09-11 — LORA's spread is structural, not accidental.** Per the Bravo/LORA team and the VMP LORA plan, LORA is organised as **three sub-teams (LORA 1, 2 and 3)** that work different features in parallel, and because most tasks are executed end-to-end, members share a baseline understanding of the whole system. That is the answer to the question this paragraph used to leave open — whether LORA's wider spread would survive a re-staffing. It is a deliberate team design, so it should.
>
> **What that does and does not change.** It does **not** fix Bravo's bus factor: Bravo's Story layer still runs through one account on the service carrying ~94% of origination. What it changes is the comparison. This finding was previously called platform-neutral; on the evidence now available it is **not** — concentration is a Bravo-specific risk, and LORA has a structural answer to it that Bravo does not. Note the basis: this is a team statement about how LORA is organised, not something measured in this pack the way the assignee counts were.
...
4. **Set the standing allocation for each platform, and name the bus factor (S, urgent).** Two separate things, and only the second is a risk finding. **The allocation** is an input the other four documents' 30/60/90 plans are sized against — they assume ~25 person-days a month of Squad S&U time, and that number needs an owner rather than an inference. **The bus factor** is the live risk: the Story layer runs through one account on the service carrying ~94% of origination, and re-staffing does not fix knowledge concentration by itself. It is **Bravo-specific** — LORA's three sub-teams and end-to-end task execution are a structural answer to the same risk (§5, updated 2026-09-11). **The allocation is not an argument about which platform should host new products** ([§5](#5-team-shape-is-a-prior-decision-not-a-platform-property)).
