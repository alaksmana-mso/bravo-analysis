...
| Largest addressable line | **Log platforms Rp 635.8M/month** across Cloud Logging (Rp 271.8M), Coralogix (Rp 253.3M) and Datadog (Rp 110.6M) — about **11×** the orchestration tier, and it is configuration. *Corrected 2026-09-13; this row previously read Rp 140.5M and 2.4×.* | Temporal Actions programme, Rp 7.2M | — | [cost §5–6](bravo-cost.md) · [logging](logging/logging-cost.md) |
| **DELIVERY** | | | | |
...
Meanwhile **Cloud Logging alone, at Rp 271.8M a month, costs 4.7× the entire tier the migration would retire** — and across all three log platforms BFI runs, Rp 635.8M, about 11×. And that is a configuration fix. *(Corrected 2026-09-13: this paragraph previously said Rp 140.5M and 2.4×, which was the production project only. See [logging/logging-cost.md](logging/logging-cost.md).)*
...
8. **Cut Bravo's log bill. Worth Rp 81–105M a month from platform configuration, available this week.** That is larger than the saving from retiring the Bravo LOS tier entirely, it needs no code change, and it also removes request bodies containing customer data from the log pipeline. *(Corrected 2026-09-13: this item previously said Rp 50–90M and attributed it to the Feign fix. That mechanism was measured on 2026-09-14 once the deployment manifests were available: real, on, and worth Rp 5–8M a month, not Rp 50–90M. The Rp 81–105M is non-production retention, exclusion filters, Cloud SQL audit logs and flow-log sampling, and it is measured.)* The code half is written: **64 pull requests across 73 repositories** — [logging/README.md](logging/README.md). And a separate question worth more than either: **Coralogix is Rp 253.3M a month and nobody has been asked what it is for.**
