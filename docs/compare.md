...
2. **`Surveyor Platform - Release reject` mapped to a code path.** 28.4% of Bravo's ticket load, growing 4.8×, unexplained. Remove it and Bravo's intervention rate is 0.152% against LORA's 0.126% — level. Days of work.
...
| **`Surveyor Platform - Release reject` maps to a defect that is then fixed** | Bravo's intervention rate falls to ≈0.152%, level with LORA's, and LORA's clearest remaining reliability edge disappears. **Days of work.** Gating measurement #2 |
...
4. **Map `Surveyor Platform - Release reject` to a code path (days).** 28.4% of Bravo's ticket load, growing 4.8×, unexplained. It decides whether Bravo's intervention rate is 3.5× LORA's or level with it.
