...
Read from `app-deployment/audit-trail/values-prod.yaml` on 14 September 2026. **This is what the running service actually uses** — a struct default in the code only applies when the variable is absent here, and where a variable is set to `""` the default never applies at all.
...
**Pull request: [bravo-audit-trail-service#36](https://github.com/bfi-finance/bravo-audit-trail-service/pull/36)** — open, not merged.  
...
**Codacy review, answered 18 September 2026.** 3 Codacy thread(s); 2 fixed in `ef3f842` (fix(logging): drop the default masked-field list; it is a deployment setting); 1 declined with the reason in the thread. Every thread is replied to and resolved on the pull request.
