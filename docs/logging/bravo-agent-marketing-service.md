# bravo-agent-marketing-service — logging findings and fixes
...
**Squad:** Agency  
**Production service:** `prod-agent-marketing`  
**Stack:** Go, zerolog, gRPC, bfi-go-pkg  
**Production volume:** 349,551 info, 65,738 warn, 23,576 error a week
...
`prod-agent-marketing` writes 349,551 info, 65,738 warn, 23,576 error over the 7 days to 13 September 2026.
...
| `GRPC_SERVER_BODY_LOGGING_ON_ERROR_ONLY` | `true` |
| `HTTP_CLIENT_BODY_JSON_MASKED_FIELDS` | `phone_number,id_number,domicile_address,email,bank_accoun…` (27 fields) |
| `GRPC_SERVER_REQUEST_BODY_JSON_MASKED_FIELDS` | `phone_number,id_number,domicile_address,email,bank_accoun…` (27 fields) |
| `GRPC_SERVER_RESPONSE_BODY_JSON_MASKED_FIELDS` | `phone_number,id_number,domicile_address,email,bank_accoun…` (27 fields) |
...
**Pull request [#829](https://github.com/bfi-finance/bravo-agent-marketing-service/pull/829) was closed on 14 September 2026, on SRE's guidance.** Its only change gave the `*_JSON_MASKED_FIELDS` config fields a default list in the struct tag. Masked fields are a per-service, per-environment setting made in `app-deployment`, not a default in every service's code — not every service handles PII, and each needs its own field list. The default was reverted on the branch, which left it identical to the base branch, so the pull request was closed rather than left open with no diff. What production actually sets is in **In the production deployment** below; the wrapper-side change is [bfi-go-pkg#175](https://github.com/bfi-finance/bfi-go-pkg/pull/175).

**Pull request: [bravo-agent-marketing-service#829](https://github.com/bfi-finance/bravo-agent-marketing-service/pull/829)** — closed 14 September 2026 (see above).  
Branch: [`fix/logging`](https://github.com/bfi-finance/bravo-agent-marketing-service/tree/fix/logging), head `de6fdd4`, branched from `master` at `2ebeb9c`.

[Files changed](https://github.com/bfi-finance/bravo-agent-marketing-service/pull/829/files) · [Commits](https://github.com/bfi-finance/bravo-agent-marketing-service/pull/829/commits) · [Compare against master](https://github.com/bfi-finance/bravo-agent-marketing-service/compare/master...fix/logging)
