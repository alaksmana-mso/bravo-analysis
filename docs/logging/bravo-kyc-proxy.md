# bravo-kyc-proxy — logging findings and fixes
...
**Production volume:** 63,122 info, 14,005 warn, 8,562 error a week
...
`prod-ms-kyc-proxy` writes 63,122 info, 14,005 warn and 8,562 error over the 7 days to 13 September 2026.
...
Read from `app-deployment/kyc-proxy/values-prod.yaml` on 14 September 2026. **This is what the running service actually uses** — a struct default in the code only applies when the variable is absent here, and where a variable is set to `""` the default never applies at all.
