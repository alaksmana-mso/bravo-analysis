...
| 11 | **Return the request id to the caller and log one decision line per rejected request** — in the wrappers, not per service: `bfi-go-pkg` fills its unused `requestID` into error bodies and adds the validation field list to the access line; `bravo-lib-logging` and `bfi-logging-spring-boot-starter` write `X-Correlation-Id` back on the response. Then each squad follows the *Why did validation fail?* section in its own file; `bravo-payment-service` is the in-house reference | Platform, then squads | 1 d wrapper + 0.5 d per service | none |
