# bravo-customer-bff-service — logging findings and fixes
...
**Squad:** Customer Mobile Apps  
**Production service:** `prod-customer-bff`  
**Stack:** Go, zerolog, gRPC, RabbitMQ, bfi-go-pkg  
**Production volume:** 239,464 warn, 171,883 info, 28,011 error a week
...
`prod-customer-bff` writes 239,464 warn, 171,883 info and 28,011 error over the 7 days to 13 September 2026.
...
Read from `app-deployment/customer-bff/values-prod.yaml` on 14 September 2026. **This is what the running service actually uses** — a struct default in the code only applies when the variable is absent here, and where a variable is set to `""` the default never applies at all.
...
| `GENERIC_JSON_MASKED_FIELDS` | `access_token,accessKey,account_number,address,address_det…` (34 fields) |
...
**Pull request [#1216](https://github.com/bfi-finance/bravo-customer-bff-service/pull/1216) was closed on 14 September 2026, on SRE's guidance.** Its only change gave the `*_JSON_MASKED_FIELDS` config fields a default list in the struct tag. Masked fields are a per-service, per-environment setting made in `app-deployment`, not a default in every service's code — not every service handles PII, and each needs its own field list. The default was reverted on the branch, which left it identical to the base branch, so the pull request was closed rather than left open with no diff. What production actually sets is in **In the production deployment** below; the wrapper-side change is [bfi-go-pkg#175](https://github.com/bfi-finance/bfi-go-pkg/pull/175).

**Pull request: [bravo-customer-bff-service#1216](https://github.com/bfi-finance/bravo-customer-bff-service/pull/1216)** — closed 14 September 2026 (see above).  
Branch: [`fix/logging`](https://github.com/bfi-finance/bravo-customer-bff-service/tree/fix/logging), head `181dcc64`, branched from `master` at `726b6ab2`.

[Files changed](https://github.com/bfi-finance/bravo-customer-bff-service/pull/1216/files) · [Commits](https://github.com/bfi-finance/bravo-customer-bff-service/pull/1216/commits) · [Compare against master](https://github.com/bfi-finance/bravo-customer-bff-service/compare/master...fix/logging)
...
It found a real defect in this change: `tagalign` on the two struct tags this change lengthened. Fixed on the branch.
