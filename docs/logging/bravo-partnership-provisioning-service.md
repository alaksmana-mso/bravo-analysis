# bravo-partnership-provisioning-service — logging findings and fixes

**Squad:** LORA Core  
**Production service:** `prod-ms-partnership-provisioning`  
**Stack:** Go, zerolog, HTTP, bfi-go-pkg  
**Production volume:** 668 warn, 3 info, 2 error a week
...
`prod-ms-partnership-provisioning` writes 668 warn, 3 info, 2 error over the 7 days to 13 September 2026.
...
**Codacy review, answered 18 September 2026.** 3 Codacy thread(s); 3 pointed at code already reverted or fixed on 14 September. Every thread is replied to and resolved on the pull request.
...
**CI caught a real defect in this change and it has been fixed:** the shared logger import had to be aliased (`bfilogger`), and then `Env.Logger()` in the same file needed the alias too. Only a Codacy coverage gate remains. Full list in [README.md](README.md#what-ci-said-about-pack-two).
