# bravo-kyc-sign-service — logging findings and fixes

**Squad:** not filed under a squad folder — cloned into `squads/others/` by this review  
**Production service:** `prod-ms-kyc-sign`  
...
`prod-ms-kyc-sign` writes no log entries over the window over the 7 days to 13 September 2026.
...
**Codacy review, answered 18 September 2026.** 5 Codacy thread(s); 3 pointed at code already reverted or fixed on 14 September; 2 declined with the reason in the thread. Every thread is replied to and resolved on the pull request.
...
**CI caught a real defect in this change and it has been fixed:** the shared logger import had to be aliased (`bfilogger`) because the `HTTPClient(logger zerolog.Logger)` parameter shadows the package. Only a Codacy coverage gate remains. Full list in [README.md](README.md#what-ci-said-about-pack-two).
